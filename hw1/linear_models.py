...
def softmax_cross_entropy_vectorized(W: np.ndarray, X: np.ndarray, y: np.ndarray):
    ...
    correct_class_prob = probs[np.arange(N), y] 
    log_probs = -np.log(correct_class_prob)
    ...
    *,
    ...
    for lr in learning_rates:
      for batch in batch_sizes:
        if verbose : 
          print(f"Training model with lr={lr}, batch={batch}")
        model = ModelClass(X_train, y_train, **model_kwargs)

        model.train(X_train, y_train, learning_rate = lr, num_iters = num_iters, batch_size = batch, verbose = False)
